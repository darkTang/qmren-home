<template>
  <div class="weather text-hidden" v-if="weatherData.adCode.city &&  weatherData.weather.weather">
  ...
  getAdcode(Key)
    .then(res => {
      if (res.status == 1) {
        ...
        // 获取天气信息
        getWeather(Key, weatherData.adCode.adcode)
          .then(res => {
            if (res.status == 1) {
              weatherData.weather = {
